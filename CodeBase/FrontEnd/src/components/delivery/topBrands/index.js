...
var topRestosList = []
for (var i = 0; i < restaurants_top_rated.length; i ++){
  topRestosList[i] = {
    id:i,
    cover: 'https://bar-photos-2021.s3.amazonaws.com/ChIJ-Rmj8xb2wokRKfvBCL7PDz8.png'//restaurants_top_rated[i].id,
  }
...
const TopBrands = ({list}) => {
  return (
    <div className="top-brands max-width">
      <div className="collection-title">Top Rated Brands</div>
      <Slider {...settings}>
        {list.map((brand) => ( //changed topBrandsList to topRestosList
          <div>
            <div className="top-brands-cover">
              <img
                className="top-brands-image"
                src={"https://bar-photos-2021.s3.amazonaws.com/"+brand.Id+".png"}
                alt={brand.Name}
              />
              <div className="top-brands-card-title">{brand.Name}</div>
            </div>
          </div>
        ))}
      </Slider>
